"""Diagnose Step 1 — per-runbook LLM rubric scoring and code finalize (from legacy eval_runbook)."""
...
# Services with no KB runbook — Step1 always novel in mock.
...
def step1_result_to_state(result, candidates: list[RunbookCandidate]) -> dict:
    selected = None
    if result.selected_doc_id:
        selected = next((c for c in candidates if c.doc_id == result.selected_doc_id), None)
    rubrics = []
...
def run_diagnose_step1(
    ...
    *,
    ...
    return step1_result_to_state(result, candidates)
