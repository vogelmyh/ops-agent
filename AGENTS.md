# Agent 工作指引（ops 面试 monorepo）

本仓库为 **运维 Agent 面试作品** 的 Git monorepo，包含 Python Agent、Simulator、Java 后端与部署清单。
...
- Java 复刻项目（`bcs-*`、`lts-*` 等）在**父目录** `project-for-interview/`，不在本 monorepo
