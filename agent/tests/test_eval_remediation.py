...
def test_verify_remediation_resolved_ecomm_manager():
    mark_remediated("ecomm-manager")
    result = verify_remediation_node(_state_after_write("ecomm-manager"))
    assert result["incident_resolved"] is True
    assert result["remediation_attempt"] == 1


def test_verify_remediation_unresolved_without_mark():
