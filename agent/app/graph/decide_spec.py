...
SCENARIO_DECIDE_ROWS: dict[tuple[str, str], MockDecideRow] = {
    ...
    ("ecomm-manager", "disk-full"): MockDecideRow(
        ...
        tool_args={"node_name": "node-ecomm-manager-0", "force": False, "delete_emptydir": False},
        tool_content="Audit log volume full; draining node to evict pods and free storage.",
        reasoning="Disk full on audit log path; runbook supports drain_node.",
        expected_needs_approval=True,
        expected_route="approve",
    ),
    ("ecomm-manager", "connection-leak"): MockDecideRow(
        ...
        reasoning="Connection leak detected; runbook recommends restart_deployment.",
        expected_needs_approval=True,
        expected_route="approve",
    ),
    ...
    ("ecomm-order", "stream-paused"): MockDecideRow(
        ...
        tool_name="restart_deployment",
        tool_args={"strategy": "rolling"},
        tool_content="Order event stream PAUSED; rolling restart to re-establish connections.",
        reasoning="Stream order-events is PAUSED; restart_deployment to reset connections.",
        expected_needs_approval=False,
        expected_route="write_tools",
    ),
    ("ecomm-order", "memory-leak"): MockDecideRow(
        ...
        reasoning="Memory leak on unchanged image; restart_deployment per runbook.",
        expected_needs_approval=False,
        expected_route="write_tools",
    ),
...
MOCK_MATRIX_EXTRAS: dict[str, MockDecideRow] = {
    ...
    "ecomm-cache": MockDecideRow(
        ...
        reasoning=(
            "OOMKilled pod with high restarts; restart_deployment is standard recovery "
